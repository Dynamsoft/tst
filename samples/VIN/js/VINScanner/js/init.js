...
Dynamsoft.CVR.CaptureVisionRouter.appendDLModelBuffer("VINTextLineRecognition-alpha", `${location.href.replace("index.html", "")}models`);
...
let init = (async function initCVR() {
  ...
  await cvRouter.initSettings("./read-vin.json");
  ...
  resultReceiver.onCapturedResultReceived = async (result) => {
    ...
    if (textLineResult?.length || barcodeResult?.length) {
      ...
      const parseData = barcodeResult?.length ? barcodeResult[0]?.bytes : textLineResult[0]?.text;

      let parsedResultItem;
      try {
        parsedResultItem = await parser.parse(parseData);
      } catch (ex) {
      ...
      const parseResultInfo = extractVinDetails(parsedResultItem);
