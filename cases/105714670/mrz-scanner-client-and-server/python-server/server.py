...
# Directory holding the client-side web app. Defaults to the shared `static/`
# folder one level up, so the Python and Kotlin backends serve the exact same
# client with no duplication of the ~46 MB engine resources.
STATIC_DIR = Path(
    os.environ.get("APP_STATIC_DIR", Path(__file__).resolve().parent.parent / "static")
).resolve()
...
if __name__ == "__main__":
    ...
    bundle_path = STATIC_DIR / "dist" / "mrz-scanner.bundle.js"
    if not bundle_path.is_file():
        print(f"\n[ERROR] Client resources not found in {STATIC_DIR / 'dist'}.")
        print("Run the setup script first, from the project root:\n")
        print("    npm install")
        print("    bash setup_resources.sh\n")
        raise SystemExit(1)

    print("\n=== Araxia Document Verification Demo (Python backend) ===")
